...
import FrequentMealSelector from '@/Components/FrequentMealSelector.vue';
...
const activeTab = ref('describe'); // 'describe' or 'frequent'
...
const handleFrequentMealSelect = async ({ meal, portionMultiplier }) => {
    isLoading.value = true;
    error.value = '';
            ...
            <div class="flex items-end sm:items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white rounded-t-3xl sm:rounded-3xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    ...
                    <div class="px-6 py-6 space-y-5">
                        ...
                        <!-- Frequent Meal Selector Tab -->
                        <div v-if="activeTab === 'frequent'">
                            <FrequentMealSelector @select="handleFrequentMealSelect" />
                            ...
                            <!-- Loading State -->
                            <div v-if="isLoading" class="mt-4 p-4 bg-indigo-50 border border-indigo-200 rounded-xl">
                                <div class="flex gap-3 items-center">
                                    <svg class="animate-spin h-5 w-5 text-indigo-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                    </svg>
                                    <p class="text-sm text-indigo-700 font-medium">Logging meal...</p>
                                </div>
                            </div>
                        </div>
                    </div>
